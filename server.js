...
app.put('/api/items/:id', (req, res) => {
  ...
  if (id >= 0 && id < books.length) {
    ...
    res.json(books[id]);  // Return the updated book
  } else {
    res.status(404).json({ message: 'Book not found' });  // Send error if invalid ID
  }
...
app.delete('/api/items/:id', (req, res) => {
  ...
  if (id >= 0 && id < books.length) {
    ...
    const deletedBook = books.splice(id, 1);  // Remove the book from the array

    res.json(deletedBook[0]);  // Return the deleted book
